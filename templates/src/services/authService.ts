...
export const signup = async (userData: IUser & { password: string }) => {
  try {
    ...
    const existingUsername = await User.findOne({ username: userData.username }).lean();
    if (existingUsername) {
...
export const forgotPassword = async (email: string): Promise<string> => {
  ...
  // await new PasswordReset({ userId: user.userId, token: hashedToken, expiresAt: Date.now() + 10 * 60 * 1000 }).save();
