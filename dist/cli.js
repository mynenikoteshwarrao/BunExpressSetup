...
const parseExistingModel = async (modelName) => {
    ...
    const schemaMatch = content.match(/const\s+\w+Schema\s*=\s*new\s+Schema<.*?>\(\{([\s\S]*?)\}/);
    if (schemaMatch) {
...
const generateCRUDRoutes = (modelName) => {
    const capitalizedName = capitalize(modelName);
    const camelCaseName = modelName.toLowerCase();
                ...
                if (hasCRUD) {
                    ...
                    if (updateCRUD) {
                        ...
                        if (crudExists.controller) {
                            ...
                            const updatedControllerContent = createBackupWithNewCode(existingController, newControllerContent, 'controller');
                            await fs.writeFile(controllerPath, updatedControllerContent);
                            console.log(colors.green(`✅ Updated controller: src/controllers/${modelName}Controller.ts`));
                        }
                        ...
                        if (crudExists.service) {
                            ...
                            const updatedServiceContent = createBackupWithNewCode(existingService, newServiceContent, 'service');
                            await fs.writeFile(servicePath, updatedServiceContent);
                            console.log(colors.green(`✅ Updated service: src/services/${modelName}Service.ts`));
                        }
                        // Regenerate routes if exists
                        if (crudExists.routes) {
                            const routePath = path.join(process.cwd(), 'src', 'routes', `${modelName}.ts`);
                            const existingRoutes = await fs.readFile(routePath, 'utf-8');
                            const newRouteContent = generateCRUDRoutes(modelName);
                            const updatedRouteContent = createBackupWithNewCode(existingRoutes, newRouteContent, 'routes');
                            await fs.writeFile(routePath, updatedRouteContent);
                            console.log(colors.green(`✅ Updated routes: src/routes/${modelName}.ts`));
                        }
                        console.log(colors.green('✅ CRUD operations updated successfully!'));
                        console.log(colors.yellow('📝 Previous code has been preserved as commented backup'));
                        console.log(colors.cyan('💡 What happened:'));
                        console.log(colors.dim('   • Old code was commented out with timestamp'));
                        console.log(colors.dim('   • New code was generated based on updated schema'));
                        console.log(colors.dim('   • You can review, merge, or clean up as needed'));
